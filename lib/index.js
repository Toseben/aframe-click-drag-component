...
z
  vec3.set(camera.components.position.x, camera.components.position.y, camera.components.position.z);

  forEachParent(camera, function (element) {

    if (element.components && element.components.position) {
      vec3.set(vec3.x + element.components.position.x, vec3.y + element.components.position.y, vec3.z + element.components.position.z);
    }
  });
}
